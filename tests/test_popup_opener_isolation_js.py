...
def test_html_code_runner_detaches_opener_and_sandboxes_preview():
    ...
    assert "setAttribute('sandbox', 'allow-scripts allow-forms allow-modals')" in body
    assert "frame.srcdoc" in body
    assert "win.document.write(code)" not in body
