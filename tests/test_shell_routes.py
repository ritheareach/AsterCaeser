...
class TestRunningInContainer:
    """Detect whether the AsterCaeser process itself runs inside a container."""
    ...
    def test_socket_without_explicit_opt_in_is_disabled(
        ...
        with socket.socket(socket.AF_UNIX) as unix_socket:
            ...
            if flag is None:
                ...
                monkeypatch.setenv("ASTERCAESER_ENABLE_HOST_DOCKER", flag)
...
class TestPackageProbeStatus:
    ...
    def test_vllm_cli_without_dist_is_external_for_update(self):
        ...
        assert "outside AsterCaeser" in status.note
    ...
    def test_apfel_does_not_use_generic_outside_astercaeser_note(self):
        status = _package_pip_update_status(
        ...
        assert status.available is False
        assert "Update this system dependency outside AsterCaeser." not in status.note
