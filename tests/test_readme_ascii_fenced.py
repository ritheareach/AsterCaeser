...
def test_readme_opens_with_wordmark_title():
    # The README must still open with a recognizable AsterCaeser title: now the
    # centered wordmark image rather than an H1 / ASCII banner.
    head = "\n".join(README.read_text(encoding="utf-8").splitlines()[:15])
    assert 'alt="AsterCaeser"' in head, "README must open with the AsterCaeser wordmark image"
