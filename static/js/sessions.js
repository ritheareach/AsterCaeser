...
let _sortMode = Storage.get('astercaeser-session-sort') || 'active'; // default to last active
...
function _deselectCurrentSession(sid) {
  ...
  uiModule.el('current-meta').textContent = 'AsterCaeser';
...
const FOLDER_STATE_KEY = 'astercaeser-folder-state';
const FOLDER_ORDER_KEY = 'astercaeser-folder-order';
...
export async function selectSession(id, { keepSidebar = false, showLoading = true, immediateLoading = false } = {}) {
  ...
  try {
    ...
    if (currentMetaEl) {
      currentMetaEl.textContent = meta ? meta.name : 'AsterCaeser';
    }
    ...
    if (window.documentModule) {
      ...
      const meta = sessions.find(s => s.id === id);
      const shouldOpen = localStorage.getItem('astercaeser-doc-open-' + id) === '1';
...
export function setSortMode(mode) {
  _sortMode = mode || null;
  if (mode) Storage.set('astercaeser-session-sort', mode);
  else Storage.remove('astercaeser-session-sort');
  renderSessionList();
}
