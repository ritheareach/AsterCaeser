...
<#
  Build a portable Windows distribution for AsterCaeser.

  Output layout:
    dist\AsterCaeser\AsterCaeser.exe
    dist\AsterCaeser\static\...
    dist\AsterCaeser\scripts\...
    dist\AsterCaeser\mcp_servers\...
    dist\AsterCaeser\services\hwfit\data\...
...
& $pyExe -m PyInstaller --noconfirm --clean --onedir --noconsole --icon=static/icon.ico --name AsterCaeser @dataArgs launcher.py
if ($LASTEXITCODE -ne 0) { Fail "PyInstaller build failed." }

Write-Host ""
Write-Host "Build complete." -ForegroundColor Green
Write-Host "Portable app folder: $PSScriptRoot\dist\AsterCaeser" -ForegroundColor Green
