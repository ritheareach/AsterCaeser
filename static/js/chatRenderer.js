...
const _ACTION_RECENTS_KEY = 'astercaeser-msg-actions-recent';
 ...
 */
const _USER_ACTION_RECENTS_KEY = 'astercaeser-user-actions-recent';
    ...
    ctxRing.addEventListener('click', (e) => {
      ...
      if (compactBtn) {
        compactBtn.addEventListener('click', async (e) => {
          ...
          compactRole.textContent = 'AsterCaeser';
...
export function addMessage(role, content, modelName, metadata) {
  try {
    ...
    var _roleText = role === 'user' ? 'You' : (isSlash || isCompacted) ? 'AsterCaeser' : modelRouteLabel(replyModels.requestedModel, resolvedModel);
