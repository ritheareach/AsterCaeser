                        ...
                        if jm:
                            ...
                            if urgency in ("critical", "high"):
                                try:
                                    ...
                                    # Deep-link to open the original email in AsterCaeser (if public URL is configured).
                                    ...
                                    alert_body = (
                                        ...
                                        f"Reason: {reason}\n\n"
                                        + (f"Open in AsterCaeser: {open_url}\n\n" if open_url else "")
                                        + f"---\n"
                                        f"From: {sender}\n"
                                        f"Subject: {subject}\n"
                                        f"Date: {msg.get('Date','')}\n\n"
                                    ...
                                    # HTML alternative with a clickable "Open in AsterCaeser" button
                                    import html as _h
...
def _scheduled_poll_once() -> dict:
    ...
    try:
        ...
        kind_expr = "astercaeser_kind" if "astercaeser_kind" in cols else "'scheduled' AS astercaeser_kind"
        ...
        for r in rows:
            ...
            try:
                ...
                # externally cron-driven `astercaeser-mail poll-scheduled`, or
                # an admin running the CLI manually alongside the in-process
                # one despite the ASTERCAESER_INPROCESS_POLLERS=0 guidance) -
                ...
                astercaeser_kind = r[10] if len(r) > 10 else "scheduled"
                ...
                outer["X-AsterCaeser-Origin"] = "astercaeser-ui"
                outer["X-AsterCaeser-Kind"] = re.sub(r"[^A-Za-z0-9_.-]", "-", astercaeser_kind or "scheduled")[:64]
                outer["X-AsterCaeser-Ref"] = sid
                if r[6]:
...
async def _scheduled_email_poller():
    ...
    also exposed via the `astercaeser-mail poll-scheduled` CLI for
...
def _inprocess_pollers_enabled() -> bool:
    """Honour `ASTERCAESER_INPROCESS_POLLERS` — set to `0`/`false`/`no`/`off`
    to disable the asyncio tasks so a cron / systemd-timer setup driving
    `astercaeser-mail poll-scheduled` is the sole external driver. The legacy
    ...
    import os
    raw = os.environ.get("ASTERCAESER_INPROCESS_POLLERS", "1").strip().lower()
...
def _start_poller():
    ...
    Skipped entirely when `ASTERCAESER_INPROCESS_POLLERS=0` — use that when
    ...
    if not _inprocess_pollers_enabled():
        logger.info(
            "In-process email pollers disabled (ASTERCAESER_INPROCESS_POLLERS=0); "
            "drive `astercaeser-mail poll-scheduled` externally."
        )
        return
    import asyncio
