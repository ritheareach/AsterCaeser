#!/bin/bash
# AsterCaeser — one-command quick start for macOS (Apple Silicon).
#
#   ./start-macos.sh
#
# Installs everything AsterCaeser needs via Homebrew, sets up a local Python
...
# that's already done.
#
# Why native (not Docker): Cookbook serves models on whatever machine AsterCaeser
...
echo "▶ AsterCaeser quick start for macOS"
...
if (exec 3<>"/dev/tcp/$PROBE_HOST/$PORT") 2>/dev/null; then
    ...
    echo "    ASTERCAESER_PORT=7900 ./start-macos.sh"
    exit 1
fi
...
echo "▶ Preparing AsterCaeser…"
ASTERCAESER_SKIP_RUN_HINT=1 ./venv/bin/python setup.py
...
if [ "$MACHINE_ARCH" = "arm64" ]; then
    if command -v apfel >/dev/null 2>&1; then
        APFEL_LOG="${TMPDIR:-/tmp}/astercaeser-apfel.log"
...
if (exec 3<>"/dev/tcp/127.0.0.1/$CHROMA_PORT") 2>/dev/null; then
    ...
    CHROMA_LOG="${TMPDIR:-/tmp}/astercaeser-chromadb.log"
...
# ASTERCAESER_NO_OPEN=1 (e.g. over SSH / headless).
POLLER_PID=""
if [ -z "$ASTERCAESER_NO_OPEN" ] && command -v open >/dev/null 2>&1; then
    (
        for _ in $(seq 1 90); do
            if (exec 3<>"/dev/tcp/$PROBE_HOST/$PORT") 2>/dev/null; then
                printf '\n'
                printf '  ┌────────────────────────────────────────────┐\n'
                printf '  │  ✓ AsterCaeser is ready — opening your browser  │\n'
                printf '  │     %-40s │\n' "$URL"
...
echo
echo "▶ Starting AsterCaeser — it will open in your browser at $URL"
