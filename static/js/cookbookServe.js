...
function _formatServeCmdPreview(cmd) {
  ...
  if (mlxDeepSeekV4Compat) {
    ...
    if (homeMatch && shortName) {
      const shimPath = `${homeMatch[1]}/.cache/astercaeser/mlx-shims/${shortName}`;
      raw = raw.replace(
  ...
  if (mlxDeepSeekV4Compat) {
    return [
      '# AsterCaeser runtime compatibility: using sanitized MLX DeepSeek-V4 shim.',
      formatted,
    ].join('\n');
  }
  return formatted;
...
function _rerenderCachedModels() {
  ...
  list.querySelectorAll('.memory-item[data-repo]').forEach(item => {
    item.addEventListener('click', (e) => {
      ...
      panelHtml += `<label class="hwfit-sf-cb"><input type="checkbox" class="hwfit-sf" data-field="prefix_cache"${sv('prefix_cache',false)?' checked':''} /> Prefix / Radix Cache${_h('vLLM: prefix caching. SGLang: RadixAttention prefix cache; when off AsterCaeser adds --disable-radix-cache.')}</label>`;
      ...
      // reflects the stored presets. Standard AsterCaeser .dropdown look, positioned
      ...
      function _showSavedConfigMenu(anchor) {
        ...
        modelSlots.forEach(({ preset: p, slotIdx }, idx) => {
          ...
          if (p.confirmedWorking) {
            ...
            badge.innerHTML = '<svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="#2ecc71" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg>';
            it.appendChild(badge);
          }
          it.appendChild(fav);
          it.appendChild(del);
