...
class TaskScheduler:
        ...
        try:
            ...
            if gate_foreground:
                ...
                if waiting and waiting.status == "queued":
                    waiting.result = "Queued — waiting for AsterCaeser to be idle…"
                    db.commit()
                ...
                async def _cancel_if_foreground_active():
                    ...
                    while True:
                        ...
                        if has_foreground_activity():
                            ...
                            logger.info("Task '%s' interrupted because AsterCaeser became active", task.name)
                            if current_task:
                                current_task.cancel()
                            return
            ...
            try:
                ...
                msg = (
                    "Paused because AsterCaeser became active"
    ...
    async def _deliver_via_email(self, output: str, task, result: str):
        ...
        try:
            ...
            msg["X-AsterCaeser-Origin"] = "astercaeser-ui"
            msg["X-AsterCaeser-Kind"] = "task"
            msg["X-AsterCaeser-Ref"] = str(task.id)
    ...
    async def _deliver_via_mcp(self, tool_name: str, task, result: str):
        ...
        args = {
            ...
            "body": result,
            "headers": {
                "X-AsterCaeser-Origin": "astercaeser-ui",
                "X-AsterCaeser-Kind": "task",
                "X-AsterCaeser-Ref": str(task.id),
            },
        }
        if recipient:
    ...
    async def stop_background_tasks_for_foreground(self, *, reason: str = "AsterCaeser became active") -> int:
        ...
        user opens or uses AsterCaeser, foreground interaction wins immediately.
