...
import { bindMenuDismiss, dismissOrRemove } from './escMenuStack.js';
  ...
  const _EMAIL_LOCAL_DRAFT_PREFIX = 'astercaeser.email.replyDraft.v1:';
  ...
  async function _resolveComposeSendAccountId() {
    const activeAccountId = window.__astercaeserActiveEmailAccount || null;
  ...
  const _docOpenKey = (sessionId) => 'astercaeser-doc-open-' + sessionId;
  const _docMinimizedKey = (sessionId) => 'astercaeser-doc-minimized-' + sessionId;
  ...
  let _astercaeserAttachMenu = null;

  function _closeAsterCaeserAttachMenu() {
    if (_astercaeserAttachMenu) {
      _astercaeserAttachMenu.remove();
      _astercaeserAttachMenu = null;
    }
  ...
  function _attachMenuOutsideClick(e) {
    if (_astercaeserAttachMenu && !_astercaeserAttachMenu.contains(e.target)) _closeAsterCaeserAttachMenu();
  }

  function _attachMenuEscape(e) {
    if (e.key !== 'Escape') return;
    _closeAsterCaeserAttachMenu();
  }

  function _positionAsterCaeserAttachMenu(anchor, menu) {
  ...
  function _astercaeserAttachLabel(item, kind) {
    if (kind === 'gallery') {
  ...
  async function _stageAsterCaeserAttachment(kind, id) {
    const doc = docs.get(activeDocId);
    if (!doc || doc.language !== 'email') return null;
    if (!doc._composeAtts) doc._composeAtts = [];
    const res = await fetch(`${API_BASE}/api/email/compose-from-astercaeser`, {
      method: 'POST',
      credentials: 'same-origin',
      headers: { 'Content-Type': 'application/json' },
  ...
  async function _stageAsterCaeserZip(items) {
    const doc = docs.get(activeDocId);
    if (!doc || doc.language !== 'email') return null;
    if (!doc._composeAtts) doc._composeAtts = [];
    const res = await fetch(`${API_BASE}/api/email/compose-from-astercaeser-zip`, {
      method: 'POST',
      credentials: 'same-origin',
      headers: { 'Content-Type': 'application/json' },
  ...
  function _afterAsterCaeserAttachmentsAdded(count, label) {
    _renderComposeAttachments();
    clearTimeout(_autoSaveDebounce);
    _autoSaveDebounce = setTimeout(() => { saveDocument({ silent: true }); }, 800);
  ...
  async function _attachAsterCaeserItem(kind, id, label, opts = {}) {
    try {
      const data = await _stageAsterCaeserAttachment(kind, id);
      if (!data) return;
      _afterAsterCaeserAttachmentsAdded(1, label || data.filename);
      if (!opts.keepOpen) _closeAsterCaeserAttachMenu();
    } catch (err) {
      console.error('Failed to attach AsterCaeser item:', err);
      if (uiModule) uiModule.showError('Failed to attach from AsterCaeser');
    }
  }

  function _selectedAsterCaeserAttachRows(menu) {
    return Array.from(menu?.querySelectorAll?.('.email-astercaeser-attach-row.is-selected') || []);
  }

  function _syncAsterCaeserAttachSelection(menu) {
    const selected = _selectedAsterCaeserAttachRows(menu);
    const bar = menu?.querySelector?.('.email-astercaeser-attach-actions');
    const count = menu?.querySelector?.('.email-astercaeser-attach-count');
    const attachBtn = menu?.querySelector?.('.email-astercaeser-attach-selected');
  ...
  async function _attachSelectedAsterCaeserItems(menu) {
    const rows = _selectedAsterCaeserAttachRows(menu);
    if (!rows.length) return;
    const btn = menu.querySelector('.email-astercaeser-attach-selected');
    if (btn) {
      btn.disabled = true;
    ...
    try {
      ...
      if (zip) {
        await _stageAsterCaeserZip(items);
        added = 1;
      } else {
        for (const item of items) {
          await _stageAsterCaeserAttachment(item.kind, item.id);
          added += 1;
        }
      }
      _afterAsterCaeserAttachmentsAdded(added, zip ? 'astercaeser-attachments.zip' : undefined);
      _closeAsterCaeserAttachMenu();
    } catch (err) {
      console.error('Failed to attach selected AsterCaeser items:', err);
      if (uiModule) uiModule.showError(added ? `Attached ${added}, then failed` : 'Failed to attach from AsterCaeser');
      _renderComposeAttachments();
    } finally {
      if (btn) {
  ...
  async function _loadAsterCaeserAttachItems(menu, kind) {
    const list = menu.querySelector('.email-astercaeser-attach-list');
    if (!list) return;
    ...
    const q = (menu.querySelector('.email-astercaeser-attach-search')?.value || '').trim();
    try {
      ...
      if (!items.length) {
        list.innerHTML = `<div class="email-astercaeser-attach-empty">${q ? 'No matches' : `No ${kind === 'gallery' ? 'images' : 'documents'}`}</div>`;
        _syncAsterCaeserAttachSelection(menu);
        return;
      }
      list.innerHTML = '';
      for (const item of items) {
        const label = _astercaeserAttachLabel(item, kind);
        const row = document.createElement('button');
        row.type = 'button';
        row.className = `email-astercaeser-attach-row ${kind === 'gallery' ? 'is-gallery' : ''}`;
        row.dataset.id = item.id || '';
        row.dataset.kind = kind;
        if (kind === 'gallery') {
          const src = item.url ? `${API_BASE}${item.url}` : '';
          row.innerHTML = `
            <span class="email-astercaeser-attach-dot" aria-hidden="true"></span>
            <span class="email-astercaeser-attach-thumb">${src ? `<img src="${_escHtml(src)}" alt="">` : ''}</span>
            <span class="email-astercaeser-attach-main">
              <span class="email-astercaeser-attach-title">${_escHtml(label)}</span>
              <span class="email-astercaeser-attach-meta">${_escHtml(item.filename || 'image')}</span>
            </span>
          `;
        } else {
          row.innerHTML = `
            <span class="email-astercaeser-attach-dot" aria-hidden="true"></span>
            <span class="email-astercaeser-attach-icon">${langIcon(item.language || 'text', 14, { style: 'opacity:0.8;' })}</span>
            <span class="email-astercaeser-attach-main">
              <span class="email-astercaeser-attach-title">${_escHtml(label)}</span>
              <span class="email-astercaeser-attach-meta">${_escHtml(item.language || 'text')}</span>
            </span>
          `;
        }
        ...
        row.addEventListener('dblclick', () => _attachAsterCaeserItem(kind, item.id, label, { keepOpen: false }));
        list.appendChild(row);
      }
      _syncAsterCaeserAttachSelection(menu);
    } catch (err) {
      console.error('Failed to load AsterCaeser attach items:', err);
      list.innerHTML = '<div class="email-astercaeser-attach-empty">Could not load</div>';
    }
  }
  ...
  function _showComposeAttachMenu(anchor) {
    ...
    menu.className = 'email-astercaeser-attach-menu';
    menu.innerHTML = `
      <button type="button" class="email-astercaeser-attach-local">
      ...
      <div class="email-astercaeser-attach-tabs">
      ...
      <label class="email-astercaeser-attach-search-wrap">
        ...
        <input type="search" class="email-astercaeser-attach-search" placeholder="Search attachments">
      </label>
      <div class="email-astercaeser-attach-list"></div>
      <div class="email-astercaeser-attach-actions">
        <span class="email-astercaeser-attach-count"></span>
        <button type="button" class="email-astercaeser-attach-selected" disabled>
    ...
    _astercaeserAttachMenu = menu;
    _positionAsterCaeserAttachMenu(anchor, menu);
    menu.querySelector('.email-astercaeser-attach-local')?.addEventListener('click', () => {
      _closeAsterCaeserAttachMenu();
      document.getElementById('doc-email-file-input')?.click();
    });
    menu.querySelectorAll('[data-ody-attach-kind]').forEach(btn => {
      btn.addEventListener('click', () => _loadAsterCaeserAttachItems(menu, btn.dataset.odyAttachKind));
    });
    let attachSearchTimer = null;
    menu.querySelector('.email-astercaeser-attach-search')?.addEventListener('input', () => {
      clearTimeout(attachSearchTimer);
      attachSearchTimer = setTimeout(() => {
        _loadAsterCaeserAttachItems(menu, menu.dataset.odyAttachKind || 'document');
      }, 220);
    });
    menu.querySelector('.email-astercaeser-attach-selected')?.addEventListener('click', () => _attachSelectedAsterCaeserItems(menu));
    setTimeout(() => {
      document.addEventListener('click', _attachMenuOutsideClick, true);
      document.addEventListener('keydown', _attachMenuEscape, true);
    }, 0);
    _loadAsterCaeserAttachItems(menu, 'document');
  }
    ...
    try {
      const res = await fetch(`${API_BASE}/api/email/draft`, {
        ...
        body: JSON.stringify({
          ...
          account_id: window.__astercaeserActiveEmailAccount || null,
        }),
      });
      const data = await res.json();
      if (data.success) {
  ...
  const _AI_REPLY_CONTEXT_STORE_PREFIX = 'astercaeser:email-ai-reply-context:v1:';
  ...
  export function openPanel() {
    ...
    let _fontIdx = parseInt(localStorage.getItem('astercaeser-doc-fontsize') || '0', 10);
    ...
    function _applyDocFont() {
      ...
      localStorage.setItem('astercaeser-doc-fontsize', _fontIdx);
    }
    _applyDocFont();
  ...
  const _DOC_RECENTS_KEY = 'astercaeser-doc-actions-recent';
  ...
  function _saveSuggestionsToStorage() {
    ...
    if (data.length) {
      localStorage.setItem('astercaeser-suggestions-' + activeDocId, JSON.stringify(data));
    } else {
      localStorage.removeItem('astercaeser-suggestions-' + activeDocId);
    }
  }
  ...
  function _restoreSuggestionsFromStorage(docId) {
    try {
      const raw = localStorage.getItem('astercaeser-suggestions-' + docId);
      if (!raw) return;
