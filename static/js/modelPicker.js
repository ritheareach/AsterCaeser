...
const RECENT_KEY = 'astercaeser-model-recent';
const FAVORITES_KEY = 'astercaeser-model-favorites';
const RECENT_MAX = 5;
...
function _initModelPickerDropdown() {
  ...
  async function _refreshLocalProbe() {
    try {
      if (window.__astercaeserChatBusy || Date.now() < (window.__astercaeserChatBusyUntil || 0)) return;
    } catch (_) {}
    const now = Date.now();
    ...
    if (all.length <= BROWSE_ALL_LIMIT) {
      ...
      sorted.forEach(provider => {
        ...
        header.addEventListener('click', (e) => {
          ...
          _saveList('astercaeser-model-collapsed', [..._collapsedProviders]);
          const st = listEl.scrollTop;
          _populate('');
          listEl.scrollTop = st;
        });
  ...
  async function _pick(m) {
    ...
    try { document.dispatchEvent(new CustomEvent('astercaeser:model-picked', { detail: m })); } catch {}
  ...
  document.addEventListener('astercaeser:auto-select-model', async (e) => {
...
export function updateModelPicker() {
  ...
  // SECURITY: deliberately NOT auto-injecting `astercaeser-model-favorites[0]`
