...
def _is_containerized():
    """Best-effort check for whether the local AsterCaeser process is running in a container."""
    if _remote_host:
        return False
...
def _hardware_visibility_warning(result):
    ...
    if not result.get("has_gpu"):
        return {
            ...
            "message": (
                "Cookbook is scanning hardware from inside the AsterCaeser container. "
