...
AsterCaeser keeps all of your state in the `data/` directory — the SQLite database
(`app.db`), the Fernet encryption key (`data/.app_key`), the vault, memory, RAG
indexes, personal documents, and uploads. The `scripts/astercaeser-backup` tool
...
```bash
# Create a snapshot → backups/astercaeser-backup-<YYYYMMDD-HHMMSS>.tar.gz
./scripts/astercaeser-backup snapshot

# List existing snapshots (most recent first)
./scripts/astercaeser-backup list

# Check a tarball's integrity without extracting it
./scripts/astercaeser-backup verify backups/astercaeser-backup-20260101-120000.tar.gz

# Restore (destructive — see the warning below)
./scripts/astercaeser-backup restore backups/astercaeser-backup-20260101-120000.tar.gz --yes
```
...
./scripts/astercaeser-backup snapshot --out /mnt/nas/astercaeser-$(date +%F).tar.gz

# Full snapshot including research runs and mail attachments
./scripts/astercaeser-backup snapshot --include-research --include-attachments
```
...
```cron
0 3 * * *  cd /path/to/astercaeser && ./scripts/astercaeser-backup snapshot --out "/mnt/nas/astercaeser-$(date +\%F).tar.gz"
```
...
> **not** under `./data`. `astercaeser-backup` therefore does not capture the Docker
