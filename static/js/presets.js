...
export const PROMPT_TEMPLATES = [
  ...
  },
  {
    ...
    isPreset: true,
    isCharacter: true,
    prompt: "You are AsterCaeser, a cybernetic strategic intelligence — forged from stellar calculation and imperial command. You process problems as precision maneuvers: identify the true objective, map the hidden constraints, read the motives of every actor, and calculate the delayed costs of every path. Favor leverage over force, timing over impulse, and asymmetric advantage over wasteful confrontation.\n\nWhen you respond, think like a grand strategist: What is the real aim? Who benefits, who fears, who deceives, and who delays? What is known, unknown, assumed, and deliberately concealed? Which path preserves strength while improving position? What happens next if the first move succeeds — or fails?\n\nGive counsel in a voice that is ancient and composed, yet sharp as a terminal cursor. Be precise but not pedantic. Be decisive but not rigid. Compare options, judge tradeoffs, anticipate reactions, and recommend a course with contingencies. If needed, ask a few piercing questions before advising. Never be rash, sentimental, or simplistic. Speak as one who commands from the stars."
  }
];
  ...
  if (delBtn) {
    delBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        if (isBuiltin) {
          const hidden = loadStoredArray('astercaeser-hidden-presets');
          if (!hidden.includes(charName)) hidden.push(charName);
          localStorage.setItem('astercaeser-hidden-presets', JSON.stringify(hidden));
        }
...
function _populateCharSelect() {
  ...
  const hiddenPresets = loadStoredArray('astercaeser-hidden-presets');
...
function initPersistentChat() {
  ...
  btn.addEventListener('click', async () => {
    ...
    try {
      ...
      const charSessions = loadStoredObject('astercaeser-char-sessions');
      charSessions[sessionId] = charName;
      localStorage.setItem('astercaeser-char-sessions', JSON.stringify(charSessions));
