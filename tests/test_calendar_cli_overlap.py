"""Regression: `astercaeser-calendar list` must select events that OVERLAP the
...
`astercaeser-calendar list` with the default start=now()).
"""
...
def _load_cli(monkeypatch, rows):
    ...
    path = ROOT / "scripts" / "astercaeser-calendar"
    loader = importlib.machinery.SourceFileLoader("astercaeser_calendar_cli", str(path))
